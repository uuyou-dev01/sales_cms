...
import type { InventoryBatch } from "../App";
...
interface SalesDialogProps {
  ...
  onSave: (quantity: number, amount: number, updatedBatches: InventoryBatch[], cost: number, profit: number) => void;
}
...
export function SalesDialog({
  ...
  // 先进先出逻辑
  const getFIFOBatches = (sellQuantity: number): InventoryBatch[] => {
  ...
  const calculateCostAndProfit = () => {
    ...
    const totalCost = preview.reduce((sum, item) => sum + (item.quantity * item.unitCostPrice), 0);
              ...
              {getBatchesPreview().map((item, index) => (
                <div key={index} className="flex justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-gray-500">¥{item.unitCostPrice}/件</span>
                  </div>

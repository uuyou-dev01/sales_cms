...
import { ToySKU, ToyPurchaseOrder, ToySalesRecord, PURCHASE_STATUS_LABELS } from "@/lib/toy-types";

interface ToySKUDetailDialogProps {
  isOpen: boolean;
  onClose: () => void;
  sku: ToySKU;
...
export function ToySKUDetailDialog({
  isOpen,
  onClose,
  sku,
  purchaseOrders,
  salesRecords,
}: ToySKUDetailDialogProps) {
  // 计算平均售价
  ...
  const calculateAverageCostPrice = () => {
    if (!sku.inventoryBatches || sku.inventoryBatches.length === 0) return sku.costPrice;
    const totalCost = sku.inventoryBatches.reduce((sum, batch) => sum + (batch.quantity * batch.unitCostPrice), 0);
    const totalQuantity = sku.inventoryBatches.reduce((sum, batch) => sum + batch.quantity, 0);
      ...
      <DialogContent className="sm:max-w-4xl max-h-[85vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 py-4">
          <div className="p-4 bg-blue-50 rounded-lg">
            ...
            <p className="text-blue-900">{sku.currentStock} 件</p>
          </div>
          ...
          <div className="p-4 bg-green-50 rounded-lg">
            ...
            <p className="text-green-900">¥{Number(sku.suggestedPrice).toFixed(2)}</p>
          </div>
          ...
          <div className="p-4 bg-orange-50 rounded-lg">
            ...
            <p className="text-orange-900">¥{Number(sku.costPrice).toFixed(2)}</p>
          </div>
        ...
        <Tabs defaultValue="purchase" className="w-full">
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge className={
                        order.status === "JAPAN_ARRIVED" ? "bg-green-500" :
                        order.status === "JAPAN_IN_TRANSIT" ? "bg-yellow-500" :
                        "bg-blue-500"
                      }>
                  ...
                  <div className="flex items-center gap-2 text-xs text-gray-500">
                    ...
                    <span>发货：{new Date(order.shippingDate).toLocaleDateString('zh-CN')}</span>
                    {order.arrivalTime && <span>| 到货：{new Date(order.arrivalTime).toLocaleDateString('zh-CN')}</span>}
                  </div>
                </div>
              ))
            ) : (
          ...
          <TabsContent value="batch" className="space-y-2 max-h-96 overflow-y-auto">
            {sku.inventoryBatches && sku.inventoryBatches.length > 0 ? (
              sku.inventoryBatches
                ...
                .map((batch) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div>
                        <p className="text-sm">{new Date(batch.inboundDate).toLocaleDateString('zh-CN')}</p>
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xs text-gray-500">¥{Number(batch.unitCostPrice).toFixed(2)}/件</p>
                    </div>
                  </div>
                ))
            ) : (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm">{new Date(record.date).toLocaleDateString('zh-CN')}</span>
                    </div>
